...
class MockTransformerPublisher:
    ...
    def publish_entry_signal(self, symbol, pattern, confidence, attention_span, atr_multiplier):
        ...
        message = {
            ...
            "timestamp": datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%S.%fZ")[:-4] + "Z",
            "pattern": {"strategy_setup": pattern, "confidence": confidence},
            "attention_metrics": {"attention_span": attention_span},
            "recommended_parameters": {"atr_multiplier": atr_multiplier},
            "regime_validation": {"veto": False},
        }
...
def test_basic_publish():
    ...
    try:
        ...
        print("✅ PASS: Basic publishing works")
        print(f"   Topic: {topic}")
        ...
        print(f"❌ FAIL: Assertion error - {e!s}")
        return False
    except Exception as e:
        print(f"❌ FAIL: Unexpected error - {e!s}")
        return False
...
def test_heartbeat_frequency():
    ...
    try:
        ...
        print("✅ PASS: Heartbeat frequency validated")
...
def test_message_structure():
    ...
    try:
        ...
        assert data["recommended_parameters"]["atr_multiplier"] > 0, (
            "ATR multiplier must be positive"
        )

        print("✅ PASS: Message structure validated")
...
def test_timestamp_format():
    ...
    try:
        ...
        try:
            ...
            print("✅ PASS: Timestamp format validated")
            print("   Format: ISO8601 UTC")
            print(f"   Timestamp: {timestamp}")
            print(f"   Age: {age:.3f}s")

        except ValueError as e:
            raise AssertionError(f"Invalid timestamp format: {timestamp} ({e!s})")
...
def run_all_tests():
    ...
    for test_name, test_func in tests:
        try:
            ...
            print(f"❌ FAIL: {test_name} - Uncaught exception: {e!s}")
