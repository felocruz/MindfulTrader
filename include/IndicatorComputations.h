...
/**
 * DetectNR7: Linda Raschke's Narrow Range 7 compression pattern detection.
 *
 * Algorithm:
 * 1. Calculate current bar range = high - low
 * 2. Calculate 7-bar average range from rangeArray
 * 3. Determine range percentile = currentRange / avg7BarRange
 * 4. Classify strength based on percentile and volume
 * 5. Return NR7Enum with quality score
 *
 * Quality Scoring:
 * - WEAK (range 95-100% of avg): base quality 0.20
 * - STRONG (range 85-95% + volume declining): base quality 0.40
 * - EXTREME (range <80% + volume dry-up): base quality 0.70
 *
 * Bonuses:
 * - Very tight range (<70%): +0.1
 * - Multi-bar consolidation (5+ bars): +0.1
 * - Volume declining: +0.15 if <80%, +0.1 if <100%
 */
    ...
    float& volumeSpike, float& qualityScore)
{
    ...
    currentRange = 0.0f;
    avg7BarRange = 0.0f;
    rangePercentile = 1.0f;
    volumeSpike = 1.0f;
    qualityScore = 0.0f;

    // Validation
    if (currentHigh <= currentLow || rangeArray.size() < 7) {
        return NR7Enum::NONE;
    }
    ...
    // Calculate 7-bar average range
    float sum = 0.0f;
    for (size_t i = 0; i < 7 && i < rangeArray.size(); i++) {
        sum += rangeArray[i];
    }
    avg7BarRange = sum / 7.0f;

    if (avg7BarRange <= 0.0f) {
        return NR7Enum::NONE;
    }

    // Calculate range percentile (0.0 = very tight, 1.0+ = wider than average)
    rangePercentile = currentRange / avg7BarRange;
    ...
    // Determine if this is the narrowest range in 7 bars
    bool isNarrowest = true;
    for (size_t i = 0; i < 7 && i < rangeArray.size(); i++) {
        if (rangeArray[i] < currentRange) {
            isNarrowest = false;
            break;
        }
    }
    ...
    if (!isNarrowest) {
        return NR7Enum::NONE;  // Not NR7
    }

    // Classify strength based on percentile
    NR7Enum nr7Enum = NR7Enum::NONE;
    float baseQuality = 0.0f;

    if (rangePercentile < 0.80f) {
        // EXTREME: <80% of 7-bar average = very tight
        nr7Enum = NR7Enum::EXTREME;
        baseQuality = 0.70f;
    }
    else if (rangePercentile < 0.95f) {
        // STRONG: 80-95% of 7-bar average
        nr7Enum = NR7Enum::STRONG;
        baseQuality = 0.40f;
    }
    else if (rangePercentile < 1.00f) {
        // WEAK: 95-100% of 7-bar average (barely narrowest)
        nr7Enum = NR7Enum::WEAK;
        baseQuality = 0.20f;
    }
    else {
        return NR7Enum::NONE;  // Not narrowest
    }

    // Calculate quality score with bonuses
    qualityScore = baseQuality;

    // Bonus 1: Very tight range (<70% of average)
    if (rangePercentile < 0.70f) {
        qualityScore += 0.1f;
    }

    // Bonus 2: Multi-bar consolidation (5+ bars in tight range)
    if (consolidationBars >= 5) {
        qualityScore += 0.1f;
    }
    else if (consolidationBars >= 3) {
        qualityScore += 0.05f;
    }

    // Bonus 3: Volume declining (low volume = compression)
    // Low volumeSpike indicates dry-up (good for compression)
    if (volumeSpike < 0.8f) {
        qualityScore += 0.15f;  // Volume well below average = strong compression
    }
    else if (volumeSpike < 1.0f) {
        qualityScore += 0.1f;   // Volume slightly below average
    }
    else if (volumeSpike < 1.2f) {
        qualityScore += 0.05f;  // Volume neutral/slightly elevated
    }
    // else: volume elevated, no bonus
