...
struct FeatureScaler {
    ...
    inline static std::array<float, N_DIMS> DIM_WINSOR_SIGMA_OVERRIDE = {  // compiled defaults; overwritten in place by LoadConfig()
        ...
        12.0f,    //  3  log_scale_expansion_ratio    RE-AUDITED 2026-09-04 (supersedes NEEDS-RE-AUDIT placeholder) against the current BV-based signal, reusing the already-completed "logscale" full-dataset run (tools/output/observation_vector_recalibration_logscale_20260904_034035.txt, all 471,930,891 ticks, no new heavy pass needed): n=18850, rate-at-bound(6.0)=0.0424%, GPD fit u=p99=2.9543, n_tail=188, xi=+0.4233 (Frechet/unbounded), sigma=0.4628. Extrapolation ratio for p=1/N here is ~100x (N/n_tail) -- used the tool's own p=1/N return level directly (11.9459, rounded to 12.0). (NOTE: this comment previously called amihud_illiquidity's own case "pathological ~5000x" as a contrast point -- that claim was a zeta_u subsampling-correction bug, since corrected; amihud is actually the same ordinary ~100x case as this dim, see dim11's own comment below.)
