...
struct FeatureScaler {
    ...
    inline static std::array<float, N_DIMS> SHRINKAGE_SCALE_MIN = {  // compiled defaults; overwritten in place by LoadConfig()
        ...
        0.000781f,  // 10  skewness_idx         RE-AUDITED 2026-09-04 (supersedes NEEDS-RE-AUDIT placeholder) against the current activity-clock signal: real full-dataset run (tools/output/observation_vector_recalibration_activity_20260904_155731.txt, all 471,930,891 ticks, corrSkewnessMadZ diagnostic) found correlation(localMad,|z|) = -0.2459 (n=4,664,244) -- a genuine collapse signature, same mechanism as dim6/dim9/dim12, NOT a real ~1381-sigma tail (raw max|z|=1761.25 with rate-at-bound(6.0)=5.43% was otherwise inexplicable: BowleySkewness is mathematically bounded to [-1,1], so a huge z can only come from a collapsed denominator). ENABLED -- floor set at the observed local-scale p1 (0.000781473), same "derive from the real observed series" convention as dim6/dim12. DIM_WINSOR_SIGMA_OVERRIDE[10] intentionally left at its 0.0f default pending a follow-up run to refit the GPD bound on the shrinkage-CORRECTED z (this run measured the raw, pre-shrinkage z only) -- do not set a bound here without that.
    ...
    inline static std::array<float, N_DIMS> DIM_WINSOR_SIGMA_OVERRIDE = {  // compiled defaults; overwritten in place by LoadConfig()
        0.0f,     //  0  log_scale_ratio   RE-AUDITED 2026-09-04 against the current BV-based signal (reusing the already-completed 2026-09-04 "logscale" full-dataset run, tools/output/observation_vector_recalibration_logscale_20260904_034035.txt -- no new heavy pass needed): n=4921, rate-at-bound(6.0)=0.0406%, GPD fit u=p99=3.838, n_tail=49, xi=-0.2520 (Weibull/bounded) -- a genuine finite endpoint ~7.38, barely above the current 6.0 default. Sample is thin (n_tail=49) and the practical difference is marginal (clip rate would drop from ~0.04% to ~0.02%) -- audited clean, current default adequate, no override forced on a thin fit. (SHRINKAGE_SCALE_MIN's own collapse-signature question for this dim is separate and still open.)
        0.0f,     //  1  burstiness_index     RE-AUDITED 2026-09-04 against the current robust-CV signal (full 471,930,392-tick population, tools/output/observation_vector_recalibration_burstiness_20260903_234906.txt): mean|z|=1.1351, rate-at-bound(6.0)=1.9557%, GPD fit u=p99=7.9865, n_tail=94196, xi=-0.0336 (Gumbel/borderline). Matches this dim's own prior real-data validation (burstiness_index reformulation finding: "mean|z|=1.14, rate-at-bound=1.96%, sane, normal clip rate") almost exactly -- audited clean, current default adequate, no override needed.
        0.0f,     //  2  relative_range
        12.0f,    //  3  log_scale_expansion_ratio    RE-AUDITED 2026-09-04 (supersedes NEEDS-RE-AUDIT placeholder) against the current BV-based signal, reusing the already-completed "logscale" full-dataset run (tools/output/observation_vector_recalibration_logscale_20260904_034035.txt, all 471,930,891 ticks, no new heavy pass needed): n=18850, rate-at-bound(6.0)=0.0424%, GPD fit u=p99=2.9543, n_tail=188, xi=+0.4233 (Frechet/unbounded), sigma=0.4628. Extrapolation ratio for p=1/N here is only ~100x (N/n_tail), a moderate, well-supported ratio (unlike amihud's pathological ~5000x) -- used the tool's own p=1/N return level directly (11.9459, rounded to 12.0) rather than the fixed-p=1e-6 correction that case needed.
        ...
        0.0f,     // 10  skewness_idx         AUDITED 2026-09-04 -- collapse signature CONFIRMED (see SHRINKAGE_SCALE_MIN's matching comment, corr(localMad,|z|)=-0.2459), shrinkage now enabled there. Left at 0.0f (generic STATE_WINSOR_SIGMA=6.0 default) intentionally, pending a follow-up run to refit this bound on the shrinkage-corrected z -- the raw pre-shrinkage z's own GPD fit (p=1/N return level 1381.03) is meaningless for this bound now that shrinkage changes what z actually is.
