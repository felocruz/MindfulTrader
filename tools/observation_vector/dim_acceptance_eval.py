...
def redundancy_check(candidate: pl.DataFrame, context: pl.DataFrame, candidate_name: str) -> None:
    joined = (
        context.sort("timestamp_us")
        .join_asof(
            candidate.sort("timestamp_us"),
            on="timestamp_us",
            strategy="nearest",
            tolerance="5m",
        )
    ...
    if len(joined) < 100:
        print(
            "  too few aligned rows to trust a correlation/tail-dependence estimate -- "
    ...
    for dim, corr, lam in rows:
        flag = (
            "  <- BAR-GATED, low values here are NOT clean evidence of independence"
            if dim in KNOWN_BAR_GATED_DIMS
            else ""
        )
    ...
    print(
        "  [no established institutional cutoff for 'too redundant' -- these are the raw "
...
def bootstrap_mean_gap_ci(top: np.ndarray, bottom: np.ndarray, n_boot: int = 2000) -> tuple:
    ...
    return (
        float(np.mean(np.abs(top)) - np.mean(np.abs(bottom))),
...
def predictive_power_directional(
    candidate: pl.DataFrame, ticks: pl.DataFrame, horizons: list
) -> None:
    ...
    for h in horizons:
        ...
        print(
            f"  {h:>4}min: n={n:<8} hit_rate={hit_rate:.4f} 95%CI=[{lo:.4f},{hi:.4f}] "
            f"p={p:.4f} ({flag}, alpha={bonferroni_alpha:.5f} for {n_tests} tests)"
        )


def predictive_power_magnitude(
    candidate: pl.DataFrame, ticks: pl.DataFrame, horizons: list, top_decile: float
) -> None:
    print(
    ...
    for h in horizons:
        ...
        if n_top < 30 or n_bot < 30:
            print(
                f"  {h:>4}min: too few samples in top/bottom decile (n_top={n_top}, n_bot={n_bot})"
            )
            continue
        gap, lo, hi = bootstrap_mean_gap_ci(fwd[top_mask], fwd[bot_mask])
        survives = (
            "SURVIVES (CI excludes 0)" if (lo > 0 or hi < 0) else "does not survive (CI includes 0)"
        )
        print(
            f"  {h:>4}min: n_top={n_top:<7} n_bot={n_bot:<7} "
...
def main():
    ...
    print(
        f"candidate '{args.candidate_name}': {len(candidate)} rows, "
