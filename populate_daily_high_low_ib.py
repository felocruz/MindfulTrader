...
def fetch_mes_daily_data(
    ib: IB, start_date: str, end_date: str, exchange: str = "CME"
) -> pd.DataFrame:
    """
    ...
    contract = Future(symbol="MES", exchange=exchange, currency="USD")
    ...
    try:
        ...
        print(
            f"✅ Found contract: {qualified.localSymbol} ({qualified.lastTradeDateOrContractMonth})"
        )
...
def main():
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing file instead of appending"
    )
    parser.add_argument(
        "--host", type=str, default="127.0.0.1", help="IB Gateway host (default: 127.0.0.1)"
    )
    parser.add_argument(
        "--port",
        type=int,
