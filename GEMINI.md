...
`ContextManager::ComputeTriggerDecisionMetrics`/`FeatureScaler` were confirmed ALREADY pure C++ (no
`sc.*` dependency at all), and the two remaining SC-coupled dim calculators (`mean_rev_z`,
