...
- [x] File `schema/PENDING_SCHEMA_CHANGES.md` entry (PSC-04), DECIDED
- [x] `regenerate_schema.sh` (found: `include/generated/mts_schema_contract_generated.h` is NOT
      actually auto-regenerated by this script despite its log output claiming so and PSC-03's own
      text asserting the same — verified by direct testing: `kObservationDim` stayed 17 after a
      real regen run while the true flatc output, `mts_schema_generated.h`, correctly showed 18.
      Hand-fixed to match; flag this for whoever owns `regenerate_schema.sh` next, it's a real gap,
      not a one-off)
- [x] Wire `fastHurstExponent` computation + `LocalRiskContext` field + `FeatureScaler` calibration rows
- [x] Native test: `test_feature_scaler.cpp` — **found it had been silently broken since
      `fast_taleb_kurtosis`'s own 17D insertion**, not just needing new dim-18 coverage: its
      hardcoded dim indices (9/12/13/14 for tail_index/liq_fragility/recurrence_rate/fractal_dim)
      were stale 16D positions, AND (far more serious) `FeatureScaler.h`'s own
      `DIM_RECURRENCE_INDEX`/`DIM_FRACTAL_INDEX` dispatch constants were ALSO still 13/14 — meaning
      production had been applying static scaling to the wrong dims since the kurtosis ship. Both
      fixed (constants → 15/16 for the current 18D layout; test indices renumbered throughout).
      `config/execution_params.json`'s `featurescaler_winsorization.dims` array had the same 16D-era
      staleness, also fixed. Full detail: `docs/superpowers/specs/2026-08-26-activity-clock-lbrnet-
      handoff.md` §11.
- [x] Full `./build_dll.sh --no-clean`; commit
