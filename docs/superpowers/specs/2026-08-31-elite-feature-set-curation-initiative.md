...
(`tools/volatility_dim_stats.h`, `tools/volatility_dim_redundancy_eval.cpp`,
...
| 2 | `burstiness_index` | IN-PENDING-FIX | Event-driven (`raschkeBurst`) | Two fixes now stacked: (a) redirect to `raschkeBurst` landed 2026-08-29; (b) **Phase 0 fix, 2026-08-31**: reformulated to robust CV `MAD/median × 1.4404199` (newly-derived Poisson-neutrality constant, not the standard 1.4826), build-verified (`test_event_velocity_engine.cpp` passing). **Recalibration tool built 2026-09-01** (`tools/burstiness_recalibration.cpp`), ran clean against real data: rate-at-current-bound(6.0)=0.0663%, shrinkage-collapse signature found (corr(localMAD,\|z\|)=-0.3638). **Blocked on a real data-granularity gap, not yet resolved**: the only historical MES data in this workspace (`mes_continuous_ticks.parquet`) is 1-second-bar-aggregated, not real per-tick data — production's `raschkeBurst` runs on genuine per-tick arrivals, so this tool's result is a real but coarser proxy, not a faithful replica. **Decided: do not update `FeatureScaler.h`'s dim1 bound/`SHRINKAGE_SCALE_MIN[1]` off this run** — treat as directional-only pending real per-tick data (ground-truthed sizing estimate for that: ~467M rows, ~3-8GB, workable with existing Arrow tooling — not yet sourced). Full detail: `SCRATCHPAD.md`'s 2026-09-01 entry | 2026-09-01 |
...
| 20 | Drift/location (return z-score) | **OUT, tested and rejected** | N/A | Offline prototype (`tools/drift_location_eval.cpp`) found hit_rate below 0.5 at every horizon; rejected before any schema/C++ commitment. Unchanged | 2026-08-30 |
| 21 | Jump/bipower-variation ratio | **CANDIDATE-VALIDATED** | N/A | Offline prototype (`tools/jump_ratio_eval.cpp`) survives — real, substantial, opposite-of-naive-hypothesis effect. Not yet promoted to a schema field; promotion decision now sits behind this doc's own Phase 1 (redundancy audit), not yet run | 2026-08-30 |
