...
|---|---|---|---|---|
| 1 | `LocalRiskContext` vs. `PredatorContext`'s HMM visibility | **ANSWERED, 2026-09-04** | **Verdict: 7 of 8 hard gates are blind to real HMM regime output; only gate 1 (Amihud) genuinely sees it.** Traced every gate's actual data source (not just its name):<br>- **Gate 1 (Amihud, `RiskManager.cpp:854-868`): HMM-AWARE.** Calls `InferenceManager::Instance().HmmState()->Dof()` directly, tightens the veto threshold p90→p75 when `dof≤4.0` (fat-tail regime detected).<br>- **Gates 2-5 (Taleb cliff/Shannon entropy/Taleb kurtosis/spread stress, `RiskManager.cpp:869-892`): BLIND.** Read only raw `LocalRiskContext` fields, no `InferenceManager`/HMM reference anywhere in `EvaluateHardGates()`.<br>- **Gates 6-8 (`ExecutionGate.cpp:24-36`): BLIND, DESPITE MISLEADING NAMES.** `paretoTopStateRatio` (gate 6) sounds like HMM top-state occupancy but is actually `1/Hill-α` (`ComputeParetoTopStateRatioProxy()`, `PositionManager.cpp:63-69`) — a raw tail-index statistic, already flagged as a naming-drift documentation debt in `docs/ADR/gate_stack_stationarity_audit_findings.md` (not yet renamed). `shannonTenureBars` (gate 7) is `ctx.regimeDuration`, a raw bar-count from `MarketClimateIndicator`, not the Student-t HMM's own posterior-state persistence. `talebSignalSigma` (gate 8) is literally `ctx.talebKurtosis` again — the exact same raw feature gate 4 already checks, just aliased under a different name.<br>**So the founding concern is confirmed, and worse than "not yet checked": most of this gate stack was never actually reading the HMM at all, and 2 of the 3 gate names that sound regime-aware are misleading.** `PredatorContext.regime` is not read by ANY of the 8 gates. | 2026-09-04 |
...
| 3 | "Tightness/cost" liquidity axis | **OPEN** | Classic market-microstructure liquidity taxonomy (Kyle 1985; Harris) has (at least) four distinct axes: **tightness** (round-trip bid-ask spread cost), depth, resiliency, immediacy. This system's current liquidity fields don't cleanly cover tightness as its own persistent signal: `amihud_illiquidity` is a price-impact-per-dollar-volume measure (closer to depth), and `spreadStress`/`liq_fragility` (`LocalRiskContext.h`) was reformulated 2026-09-02/03 into a median-based *elasticity ratio* (closer to resiliency) — NOT the raw current bid-ask spread cost. The only place raw spread actually appears is `PositionManager.cpp`'s admission-time `SPREAD_BREACH` gate (`spreadTicks = (ask-bid)/tickSize` vs. an adaptive `allowedSpreadTicks` limit, itself widened/tightened by `spreadStress`/`raschkeBurst`) — a point-in-time execution filter, not a persistent context field anywhere else (not in `LocalRiskContext`, not in the observation vector, no rolling/percentile treatment, no `RiskManager` hard gate). Question not yet answered: does tightness deserve its own dedicated, persistent signal (e.g. a session-aware rolling percentile of realized spread cost, mirroring Amihud's own Layer B treatment) feeding `RiskGateContext`/`RiskManager`, or is the existing admission-time-only `SPREAD_BREACH` gate sufficient because tightness only matters at the instant of order placement (unlike depth/resiliency, which matter continuously)? Investigate before assuming either answer. | 2026-09-03 (queued, not started) |
...
| 6 | Pareto top-state ratio (Hill-α proxy) | `ExecutionGate.cpp:24-26` | Stationary (naming debt noted, deferred) | **NO, AND MISLEADINGLY NAMED** — actually `1/Hill-α` (`PositionManager.cpp:63-69`), not an HMM state-occupancy probability despite the name |
...
| 8 | Taleb-signal-sigma | `ExecutionGate.cpp:34-36` | Stationary; value-drift bug fixed (Task 2, that plan) | **NO** — literally `ctx.talebKurtosis` again, aliased under a different name (same raw feature as gate 4) |
...
1. **Fix the two misleading gate names first, before any regime-wiring work** (gates 6/8). This is
   cheap, mechanical, and already scoped by `docs/ADR/gate_stack_stationarity_audit_findings.md`
   (which flagged it but deferred it as "low-priority... out of scope"). Reclassify that priority now:
   leaving `paretoTopStateRatio` named as if it were an HMM state-occupancy probability actively
   obscures how blind this gate stack really is — the name itself was part of why this took an
   explicit trace to uncover, not just inspection. Touches `PositionManager.cpp` (2 call sites),
   `ExecutionGate.cpp`, `ContextManager.h`, and the wire schema field
   `RiskGateContext.pareto_top_state_ratio` — a schema change, coordinate accordingly.
2. **Gate 8 is a bare duplicate of gate 4** (`talebSignalSigma` == `talebKurtosis`, checked against a
   different threshold under a different name) — decide whether this is intentional (two independent
   thresholds on purpose, e.g. a softer early-warning + a harder halt) or accidental duplication before
   doing anything else with it. Not yet determined which.
3. **Extend gate 1's own proven pattern (`InferenceManager::Instance().HmmState()->Dof()`) to gates
   ...
   gap.
4. **Do not assume gates 6-8 need new regime-wiring once gate 8's duplication (item 2) is resolved** —
   if gate 8 turns out to be redundant with gate 4, wiring gate 4 (item 3) may already cover it;
   confirm before doing double work.
