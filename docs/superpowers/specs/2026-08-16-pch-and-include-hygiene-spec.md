...
## Approach (agreed): Compiler-Driven Strip-and-Fix
...
1. **Trim `include/MindfulTrader_Precompiled.h`** to only the verified-stable content: `sierrachart.h`,
   the existing STL header list, `nlohmann/json.hpp`, `flatbuffers/flatbuffers.h` +
   ...
   wrong during the audit). Remove all 20 project headers listed in Purpose above.
2. **Full clean rebuild, fix per-file compile errors mechanically.** `./build_dll.sh` (clean, not
   `--no-clean`) will emit "unknown type / undeclared identifier / no member" errors, file by file,
   for every `.cpp` that was implicitly relying on the PCH's blanket project-header inclusion. Add the
   exact explicit `#include` each error names to that specific file. Iterate to a fully green clean
   build. Expect this to touch most of the ~35 `.cpp` files in the target to some degree — each fix is
   mechanical and proven correct by the file compiling afterward, not a design judgment call.
3. **Measure, don't assume, the outcome.** Re-run the exact probe from Investigation Log (touch
...
4. **Re-check clangd.** Confirm the diagnostics that started this investigation
   ...
   right-sized PCH should let clangd's own PCH consumption work as intended.
...
- **Restructuring how `target_precompile_headers()` itself is invoked in `CMakeLists.txt`** (e.g.,
  per-file PCH exclusion via `SKIP_PRECOMPILE_HEADERS`) — the existing project-wide, single-PCH
  structure is kept; only its *contents* change. If Step 2 reveals a real need for
  `SKIP_PRECOMPILE_HEADERS` on specific files, treat that as new information requiring a return to
...
1. `MindfulTrader_Precompiled.h` contains zero project-authored headers — only system/STL/stable
   third-party content.
2. Every `.cpp` file explicitly includes every project header it directly uses — no file relies on
   transitive inclusion via the PCH for a project header.
3. Touching any previously-PCH-bundled project header no longer forces a full-target rebuild — only
   its actual dependents recompile.
4. `build_dll.sh` and the Python integration test suite both green.

## Rollback

Purely additive/corrective changes to header contents (`#include` lines) and one file's precompile
list — no runtime logic touched. Revert `MindfulTrader_Precompiled.h` and any added `#include` lines
as a single commit if the strip-and-fix pass surfaces something unexpected; the pre-existing mega-PCH
behavior returns immediately since `CMakeLists.txt`'s `target_precompile_headers()` call itself is
unchanged.
